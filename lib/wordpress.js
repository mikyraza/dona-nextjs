import { magazines as initialMagazines } from '../app/magazines/data.js';
import {
  dbGetArticles,
  dbGetArticleById,
  dbUpsertArticle,
...
import { slugify, getArticleSlug } from './slugHelper.js';
    ...
    for (const art of localArts) {
      if (art && art.id && !seenIds.has(art.id.toString())) {
        ...
        merged.push({
          ...art,
          id: art.id.toString(),
          slug: art.slug ? slugify(art.slug) : getArticleSlug(art),
  ...
  try {
    ...
    if (local) {
      return {
        ...
        slug: local.slug ? slugify(local.slug) : getArticleSlug(local),
...
export async function createOrUpdateArticle(articleData) {
  const newArticle = {
    ...
    slug: articleData.slug ? slugify(articleData.slug) : getArticleSlug(articleData),
...
export const DEFAULT_THEMATIC_DOSSIERS = [
  {
    ...
    status: 'Published',
    pdfUrl: '/assets/core/docs/workbook-complex-systems.pdf',
    fileSize: '4.2 MB'
  },
  {
    id: 'dos-02',
