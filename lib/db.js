...
import { hashPassword, verifyPassword, generateSecureId, isBcryptHash } from './passwordHelper.js';
import { slugify } from './slugHelper.js';
...
const DB_PATH = process.env.DATABASE_PATH || (isVercel ? '/tmp/dona.db' : path.join(process.cwd(), 'lib', 'dona.db'));
...
export function getDatabase() {
  ...
  try {
    ...
    try { db.exec('ALTER TABLE dossiers ADD COLUMN pdf_url TEXT'); } catch (e) {}
    try { db.exec('ALTER TABLE dossiers ADD COLUMN file_size TEXT'); } catch (e) {}
    ...
    // 10. Orders Table (Transactions & Subscriptions)
    db.exec(`
      CREATE TABLE IF NOT EXISTS orders (
        ...
        billing_cycle TEXT DEFAULT 'mensuel',
    ...
    // 11. Config Backups Table (Server-side Global Config Snapshots)
    db.exec(`
      ...
      CREATE INDEX IF NOT EXISTS idx_config_backups_checksum ON config_backups(checksum);
    `);
...
export function dbGetArticleById(id) {
  ...
  let row = db.prepare('SELECT * FROM articles WHERE id = ? OR slug = ?').get(strId, strId);
  if (!row) {
    // Intelligent fallback: check if any article matches slugified title or slug
    const cleanQuery = slugify(strId);
    if (cleanQuery) {
      const all = db.prepare('SELECT * FROM articles').all();
      row = all.find(a => (a.slug && slugify(a.slug) === cleanQuery) || slugify(a.title) === cleanQuery);
    }
  }
...
export function dbUpsertArticle(art) {
  ...
  // Automatically generate clean slug if omitted or empty
  let targetSlug = art.slug ? slugify(art.slug) : slugify(art.title || id);
  if (!targetSlug) targetSlug = id;

  // Verify slug uniqueness (excluding current article id)
  const existingWithSlug = db.prepare('SELECT id FROM articles WHERE slug = ? AND id != ?').get(targetSlug, id);
  if (existingWithSlug) {
...
export function dbGetVideos(filters = {}) {
  ...
  if (filters.magazine && filters.magazine !== 'all') {
    const cleanMag = filters.magazine.replace(/^magazine-\d{2}-/, '');
    conditions.push('(LOWER(magazine) = LOWER(?) OR LOWER(magazine) = LOWER(?))');
    params.push(filters.magazine, cleanMag);
  }
...
export function dbUpsertDossier(d) {
  ...
  const stmt = db.prepare(`
    INSERT INTO dossiers (
      ...
      articles, is_vip_only, is_featured, category, magazine, label, status,
      pdf_url, file_size, updated_at
    ) VALUES (
      ?, ?, ?, ?, ?, ?,
      ?, ?, ?, ?, ?, ?, ?,
      ?, ?, CURRENT_TIMESTAMP
  ...
  stmt.run(
    ...
    d.status || 'Published',
    d.pdfUrl || d.pdf_url || null,
    d.fileSize || d.file_size || null
  );
...
export function dbUpsertUser(u) {
  ...
  const id = existing ? existing.id : String(u.id || generateSecureId('usr'));
  
  // Secure Password Hashing: hash new passwords or preserve existing hash/fallback
  let password = existing?.password;
  if (u.password) {
    password = hashPassword(u.password);
  } else if (!password) {
    password = hashPassword('dona2026');
  }
...
export function dbUpdateUserPassword(idOrEmail, newPassword) {
  if (!idOrEmail || !newPassword) return false;
  const db = getDatabase();
  const hashedPassword = hashPassword(newPassword);
  db.prepare('UPDATE users SET password = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? OR lower(email) = lower(?)')
    .run(hashedPassword, String(idOrEmail), String(idOrEmail).trim());
  liveSyncXampp('UPDATE users SET password = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? OR lower(email) = lower(?)',
    [hashedPassword, String(idOrEmail), String(idOrEmail).trim()]);
  return true;
}

export function migrateAllPasswordsToBcrypt() {
  const db = getDatabase();
  const users = db.prepare('SELECT id, email, password FROM users').all();
  let migratedUsers = 0;
  for (const u of users) {
    if (u.password && !isBcryptHash(u.password) && !u.password.startsWith('scrypt$')) {
      const hashed = hashPassword(u.password);
      db.prepare('UPDATE users SET password = ? WHERE id = ?').run(hashed, u.id);
      liveSyncXampp('UPDATE users SET password = ? WHERE id = ?', [hashed, u.id]);
      migratedUsers++;
    }
  }

  const members = db.prepare('SELECT id, email, password FROM members').all();
  let migratedMembers = 0;
  for (const m of members) {
    if (m.password && !isBcryptHash(m.password) && !m.password.startsWith('scrypt$')) {
      const hashed = hashPassword(m.password);
      db.prepare('UPDATE members SET password = ? WHERE id = ?').run(hashed, m.id);
      liveSyncXampp('UPDATE members SET password = ? WHERE id = ?', [hashed, m.id]);
      migratedMembers++;
    }
  }

  return { migratedUsers, migratedMembers };
}
...
export function dbUpsertMember(m) {
  ...
  const id = existing ? existing.id : String(m.id || generateSecureId('mem'));
...
// -------------------------------------------------------------
// Database Disk Integrity Checks
// -------------------------------------------------------------
export function dbCheckIntegrity() {
  try {
    const db = getDatabase();
    const pragmaRes = db.prepare('PRAGMA integrity_check').all();
    const isOk = Array.isArray(pragmaRes) && pragmaRes.length > 0 && pragmaRes[0].integrity_check === 'ok';

    // Verify key tables are present and queryable
    const expectedTables = ['settings', 'articles', 'magazines', 'videos', 'users', 'members', 'orders', 'config_backups'];
    const tableRows = db.prepare("SELECT name FROM sqlite_master WHERE type='table'").all();
    const existingTableNames = new Set(tableRows.map(t => t.name));
    ...
    return {
      status: isOk && missingTables.length === 0 ? 'ok' : 'degraded',
      sqliteIntegrity: isOk ? 'ok' : (pragmaRes?.[0]?.integrity_check || 'error'),
...
// -------------------------------------------------------------
// Config Backups Database Operations
// -------------------------------------------------------------
export function dbRecordConfigBackup({ id, filename, filePath, checksum, sizeBytes, settingsCount, configJson }) {
  const db = getDatabase();
  db.prepare(`
    INSERT INTO config_backups (id, filename, file_path, checksum, size_bytes, settings_count, config_json, created_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
    ON CONFLICT(id) DO UPDATE SET
      checksum = excluded.checksum,
      size_bytes = excluded.size_bytes,
      settings_count = excluded.settings_count,
  ...
  `).run(
    ...
    String(checksum),
    Number(sizeBytes) || 0,
    Number(settingsCount) || 0,
    typeof configJson === 'string' ? configJson : JSON.stringify(configJson)
  );

  liveSyncXampp(
    'INSERT INTO config_backups (id, filename, file_path, checksum, size_bytes, settings_count, config_json, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, NOW()) ON DUPLICATE KEY UPDATE checksum = VALUES(checksum), size_bytes = VALUES(size_bytes), settings_count = VALUES(settings_count), config_json = VALUES(config_json)',
    [String(id), String(filename), String(filePath), String(checksum), Number(sizeBytes) || 0, Number(settingsCount) || 0, typeof configJson === 'string' ? configJson : JSON.stringify(configJson)]
  );

  return true;
}

export function dbGetConfigBackups(limit = 20) {
  const db = getDatabase();
  return db.prepare(`
    SELECT id, filename, file_path as filePath, checksum, size_bytes as sizeBytes, settings_count as settingsCount, created_at as createdAt
    FROM config_backups
...
export function dbGetLatestConfigBackup() {
  const db = getDatabase();
  return db.prepare(`
    SELECT id, filename, file_path as filePath, checksum, size_bytes as sizeBytes, settings_count as settingsCount, config_json as configJson, created_at as createdAt
    FROM config_backups
    ORDER BY created_at DESC
    LIMIT 1
  `).get() || null;
...
export function dbGetConfigBackupById(id) {
  const db = getDatabase();
  return db.prepare(`
    SELECT id, filename, file_path as filePath, checksum, size_bytes as sizeBytes, settings_count as settingsCount, config_json as configJson, created_at as createdAt
    FROM config_backups
...
export function dbRestoreConfigFromBackup(backupId) {
  const db = getDatabase();
  let backup = null;
  if (!backupId || backupId === 'latest') {
    backup = dbGetLatestConfigBackup();
  } else {
    backup = dbGetConfigBackupById(backupId);
  }

  if (!backup || !backup.configJson) {
    throw new Error(`Backup "${backupId}" not found`);
  }

  let parsed = null;
  try {
    parsed = JSON.parse(backup.configJson);
  } catch (e) {
    throw new Error(`Backup "${backupId}" contains corrupted JSON: ${e.message}`);
  }
  ...
  return {
    ...
    restoredSettingsCount: Object.keys(parsed).length,
    timestamp: Date.now()
  };
}
...
`;
    ...
    sql += `\n-- 10. Orders & Transactions\nDROP TABLE IF EXISTS \`orders\`;\nCREATE TABLE \`orders\` (\n  \`id\` VARCHAR(100) NOT NULL,\n  \`user_id\` VARCHAR(100) DEFAULT NULL,\n  \`customer_name\` VARCHAR(255) NOT NULL,\n  \`customer_email\` VARCHAR(255) NOT NULL,\n  \`plan\` VARCHAR(100) NOT NULL,\n  \`plan_name\` VARCHAR(255) NOT NULL,\n  \`billing_cycle\` VARCHAR(50) DEFAULT 'mensuel',\n  \`subtotal\` DECIMAL(10,2) NOT NULL,\n  \`tax\` DECIMAL(10,2) NOT NULL,\n  \`discount\` DECIMAL(10,2) DEFAULT 0.00,\n  \`promo_code\` VARCHAR(100) DEFAULT NULL,\n  \`total\` DECIMAL(10,2) NOT NULL,\n  \`currency\` VARCHAR(10) DEFAULT 'EUR',\n  \`payment_method\` VARCHAR(100) DEFAULT 'Carte bancaire',\n  \`card_brand\` VARCHAR(50) DEFAULT 'VISA',\n  \`card_last4\` VARCHAR(10) DEFAULT '4242',\n  \`billing_address\` TEXT DEFAULT NULL,\n  \`status\` VARCHAR(50) DEFAULT 'Completed',\n  \`next_billing_date\` DATETIME DEFAULT NULL,\n  \`created_at\` DATETIME DEFAULT CURRENT_TIMESTAMP,\n  \`updated_at\` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,\n  PRIMARY KEY (\`id\`),\n  KEY \`idx_orders_customer_email\` (\`customer_email\`),\n  KEY \`idx_orders_status\` (\`status\`),\n  KEY \`idx_orders_created_at\` (\`created_at\`)\n) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;\n\n`;
    try {
      const orders = db.prepare('SELECT * FROM orders').all();
      for (const o of orders) {
        sql += `INSERT INTO \`orders\` (\`id\`, \`user_id\`, \`customer_name\`, \`customer_email\`, \`plan\`, \`plan_name\`, \`billing_cycle\`, \`subtotal\`, \`tax\`, \`discount\`, \`promo_code\`, \`total\`, \`currency\`, \`payment_method\`, \`card_brand\`, \`card_last4\`, \`billing_address\`, \`status\`, \`next_billing_date\`, \`created_at\`, \`updated_at\`) VALUES (${escapeSqlVal(o.id)}, ${escapeSqlVal(o.user_id)}, ${escapeSqlVal(o.customer_name)}, ${escapeSqlVal(o.customer_email)}, ${escapeSqlVal(o.plan)}, ${escapeSqlVal(o.plan_name)}, ${escapeSqlVal(o.billing_cycle)}, ${escapeSqlVal(o.subtotal)}, ${escapeSqlVal(o.tax)}, ${escapeSqlVal(o.discount)}, ${escapeSqlVal(o.promo_code)}, ${escapeSqlVal(o.total)}, ${escapeSqlVal(o.currency)}, ${escapeSqlVal(o.payment_method)}, ${escapeSqlVal(o.card_brand)}, ${escapeSqlVal(o.card_last4)}, ${escapeSqlVal(o.billing_address)}, ${escapeSqlVal(o.status)}, ${escapeSqlVal(o.next_billing_date)}, ${escapeSqlVal(o.created_at)}, ${escapeSqlVal(o.updated_at)});\n`;
      }
    } catch (e) {}

    // 11. Config Backups
    sql += `\n-- 11. Config Backups\nDROP TABLE IF EXISTS \`config_backups\`;\nCREATE TABLE \`config_backups\` (\n  \`id\` VARCHAR(64) NOT NULL,\n  \`filename\` VARCHAR(255) NOT NULL,\n  \`file_path\` VARCHAR(255) NOT NULL,\n  \`checksum\` VARCHAR(64) NOT NULL,\n  \`size_bytes\` INT NOT NULL DEFAULT 0,\n  \`settings_count\` INT NOT NULL DEFAULT 0,\n  \`config_json\` LONGTEXT NOT NULL,\n  \`created_at\` DATETIME DEFAULT CURRENT_TIMESTAMP,\n  PRIMARY KEY (\`id\`),\n  KEY \`idx_config_backups_created_at\` (\`created_at\`),\n  KEY \`idx_config_backups_checksum\` (\`checksum\`)\n) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;\n\n`;
    try {
      const configBackups = db.prepare('SELECT * FROM config_backups ORDER BY created_at DESC LIMIT 50').all();
      for (const b of configBackups) {
        sql += `INSERT INTO \`config_backups\` (\`id\`, \`filename\`, \`file_path\`, \`checksum\`, \`size_bytes\`, \`settings_count\`, \`config_json\`, \`created_at\`) VALUES (${escapeSqlVal(b.id)}, ${escapeSqlVal(b.filename)}, ${escapeSqlVal(b.file_path)}, ${escapeSqlVal(b.checksum)}, ${b.size_bytes || 0}, ${b.settings_count || 0}, ${escapeSqlVal(b.config_json)}, ${escapeSqlVal(b.created_at)});\n`;
      }
    } catch (e) {}
...
export function dbCreateOrder(orderData) {
  const db = getDatabase();
  const now = new Date();
  const year = now.getFullYear();
  const randSuffix = Math.floor(100000 + Math.random() * 900000);
  const id = orderData.id || `DONA-ORD-${year}-${randSuffix}`;

  const nextBilling = new Date(now);
  if (orderData.billingCycle === 'annuel' || orderData.billing_cycle === 'annuel') {
  ...
  const order = {
    ...
    customer_name: orderData.customerName || orderData.customer_name || 'Membre DONA',
    customer_email: (orderData.customerEmail || orderData.customer_email || '').trim().toLowerCase(),
    plan: orderData.plan || 'Premium',
    plan_name: orderData.planName || orderData.plan_name || `Abonnement DONA — ${orderData.plan || 'Premium'}`,
    billing_cycle: orderData.billingCycle || orderData.billing_cycle || 'mensuel',
    subtotal: Number(orderData.subtotal != null ? orderData.subtotal : (Number(orderData.total || 29) / 1.2).toFixed(2)),
    tax: Number(orderData.tax != null ? orderData.tax : (Number(orderData.total || 29) - Number(orderData.total || 29) / 1.2).toFixed(2)),
    discount: Number(orderData.discount || 0),
    promo_code: orderData.promoCode || orderData.promo_code || null,
    total: Number(orderData.total != null ? orderData.total : 29),
    currency: orderData.currency || 'EUR',
    payment_method: orderData.paymentMethod || orderData.payment_method || 'Carte bancaire',
    card_brand: orderData.cardBrand || orderData.card_brand || 'VISA',
    card_last4: String(orderData.cardLast4 || orderData.card_last4 || '4242'),
    billing_address: orderData.billingAddress || orderData.billing_address || null,
    status: orderData.status || 'Completed',
    next_billing_date: orderData.nextBillingDate || orderData.next_billing_date || nextBilling.toISOString()
  };

  const stmt = db.prepare(`
    INSERT INTO orders (
      id, user_id, customer_name, customer_email, plan, plan_name,
      billing_cycle, subtotal, tax, discount, promo_code, total,
      currency, payment_method, card_brand, card_last4, billing_address,
      status, next_billing_date, created_at, updated_at
    ) VALUES (
      ?, ?, ?, ?, ?, ?,
      ?, ?, ?, ?, ?, ?,
      ?, ?, ?, ?, ?,
  ...
  stmt.run(
    order.id, order.user_id, order.customer_name, order.customer_email, order.plan, order.plan_name,
    order.billing_cycle, order.subtotal, order.tax, order.discount, order.promo_code, order.total,
    order.currency, order.payment_method, order.card_brand, order.card_last4, order.billing_address,
    order.status, order.next_billing_date
  );
  ...
  // Synchronize member/user plan if email exists
  if (order.customer_email) {
    try {
      const user = db.prepare('SELECT id FROM users WHERE LOWER(email) = ?').get(order.customer_email);
      if (user) {
        db.prepare("UPDATE users SET plan = ?, status = 'Actif', updated_at = CURRENT_TIMESTAMP WHERE id = ?")
          .run(order.plan, user.id);
      }
      const member = db.prepare('SELECT id FROM members WHERE LOWER(email) = ?').get(order.customer_email);
      if (member) {
        db.prepare("UPDATE members SET plan = ?, status = 'Active', updated_at = CURRENT_TIMESTAMP WHERE id = ?")
          .run(order.plan, member.id);
      }
    } catch (e) {
      console.warn('Error synchronizing member subscription with order:', e);
    }
  }

  try { exportDatabaseToSqlFile(); } catch (e) {}

  return dbGetOrderById(order.id);
}

export function dbGetOrderById(id) {
  if (!id) return null;
  const db = getDatabase();
...
export function dbGetOrdersByEmail(email) {
  if (!email) return [];
  const db = getDatabase();
  const rows = db.prepare('SELECT id FROM orders WHERE LOWER(customer_email) = ? ORDER BY created_at DESC')
    .all(email.toLowerCase().trim());
  return rows.map(r => dbGetOrderById(r.id));
}

export function dbGetOrders(limit = 50) {
  const db = getDatabase();
  const rows = db.prepare('SELECT id FROM orders ORDER BY created_at DESC LIMIT ?').all(limit);
  return rows.map(r => dbGetOrderById(r.id));
}
