...
export default function TodayArticleClient({ initialArticle = null, articleSlug = '' }) {
  const { t, currentLangObj } = useLanguage();
  const initialMatch = initialArticle ? normalizeArticle(initialArticle) : (articleSlug ? normalizeArticle(findMatchingArticle(DEFAULT_ARTICLES, articleSlug)) : null);
  ...
  useEffect(() => {
    ...
    // Step 1: Immediate check against default static articles
    let currentMatch = initialArticle || findMatchingArticle(DEFAULT_ARTICLES, articleSlug);

    // Step 2: Check localStorage if available
    try {
      if (typeof window !== 'undefined') {
        ...
        if (saved) {
          ...
          const localMatch = findMatchingArticle(combined, articleSlug);
          if (localMatch) currentMatch = localMatch;
        }
      }
    } catch (e) {
    ...
    if (currentMatch) {
      setArticle(normalizeArticle(currentMatch));
      setLoading(false);
    }

    // Step 3: Primary source of truth: Relational Database (/api/today)
    fetch('/api/today')
      .then(res => res.json())
      .then(dbConfig => {
        if (dbConfig) {
          ...
          const dbMatch = findMatchingArticle(dbCombined, articleSlug);
          if (dbMatch) {
            setArticle(normalizeArticle(dbMatch));
          } else if (!currentMatch) {
            const fallback = findMatchingArticle(DEFAULT_ARTICLES, articleSlug);
            setArticle(fallback ? normalizeArticle(fallback) : null);
          }
        }
      })
      .catch(err => {
        console.error("Error loading Today article from database:", err);
        if (!currentMatch) {
          const fallback = findMatchingArticle(DEFAULT_ARTICLES, articleSlug);
          setArticle(fallback ? normalizeArticle(fallback) : null);
        }
  ...
  if (!article) {
    return (
      <main style={{ background: "var(--color-bg)", minHeight: "90vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "20px" }}>
        ...
        <Link href="/today" style={{ textDecoration: "none", background: "#ce0028", color: "white", padding: "10px 20px", borderRadius: "2px", fontWeight: "600", fontSize: "12px", letterSpacing: "0.05em", textTransform: "uppercase" }}>
          {t('breadcrumb_back_to_today')}
        </Link>
      </main>
    );
  }
